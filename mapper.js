(function() {
    ...
    function initializeStepTracker() {
        ...
        for (var i = 0; i < steps.length; i++) {
            ...
            html += '<div class="step-circle">' + steps[i].number + '</div>';
    ...
    function updateStepTracker(step) {
        ...
        for (var j = 0; j < stepConnectors.length; j++) {
            if (j < step) stepConnectors[j].classList.add('completed');
            else stepConnectors[j].classList.remove('completed');
        }
    }
    ...
    function waitForElement(selector, callback) {
        var checkInterval = setInterval(function() {
            ...
            if (element) { clearInterval(checkInterval); callback(element); }
        }, 100);
    }
    ...
    function init() {
        waitForElement('#uploadBox', function(uploadBox) {
            uploadBox.addEventListener('click', function() { document.getElementById('fileInput').click(); });
        });
        waitForElement('#fileInput', function(fileInput) {
            fileInput.addEventListener('change', handleFileUpload);
        });
        waitForElement('#categoryInput', function(categoryInput) {
            categoryInput.addEventListener('keypress', function(e) { if (e.key === 'Enter') addCategory(); });
        });
        waitForElement('#addCategoryBtn', function(btn) { btn.addEventListener('click', addCategory); });
        waitForElement('#startMappingBtn', function(btn) { btn.addEventListener('click', startMapping); });
        waitForElement('#startConstituentMappingBtn', function(btn) { btn.addEventListener('click', startConstituentMapping); });
        waitForElement('#startGiftTypeMappingBtn', function(btn) { btn.addEventListener('click', startGiftTypeMapping); });
        waitForElement('#categoriesList', function(list) {
            list.addEventListener('click', function(e) {
                if (e.target.tagName === 'BUTTON') removeCategory(parseInt(e.target.getAttribute('data-index')));
            });
        });
        document.addEventListener('click', function(e) {
            if (e.target.classList.contains('category-btn')) {
                ...
                if (mappingType === 'event') selectCategory(appeal, category);
                else if (mappingType === 'spotlight') selectSpotlight(appeal, category);
                else if (mappingType === 'constituent') selectConstituentType(appeal, category);
                else if (mappingType === 'gifttype') selectGiftType(appeal, category);
            } else if (e.target.classList.contains('nav-btn')) {
                var action = e.target.getAttribute('data-action');
                var mappingType = e.target.getAttribute('data-mapping-type');
                if (mappingType === 'event') { if (action === 'previous') previousAppeal(); else nextAppeal(); }
                else if (mappingType === 'spotlight') { if (action === 'previous') previousSpotlight(); else nextSpotlight(); }
                else if (mappingType === 'constituent') { if (action === 'previous') previousConstituentType(); else nextConstituentType(); }
                else if (mappingType === 'gifttype') { if (action === 'previous') previousGiftType(); else nextGiftType(); }
            }
        });
    }
    ...
    function handleFileUpload(e) {
        ...
        if (detectedIndustry) setSpotlightConfig(detectedIndustry);
        ...
        reader.onload = function(e) {
            try {
                ...
                for (var i = 0; i < giftJsonData.length; i++) {
                    if (giftJsonData[i]['Gift Appeal']) uniqueAppeals[giftJsonData[i]['Gift Appeal']] = true;
                }
                giftAppeals = Object.keys(uniqueAppeals).sort();

                // Extract Gift Types
                var uniqueGiftTypes = {};
                for (var g = 0; g < giftJsonData.length; g++) {
                    if (giftJsonData[g]['Gift Type']) uniqueGiftTypes[giftJsonData[g]['Gift Type']] = true;
                }
                ...
                for (var j = 0; j < constituentJsonData.length; j++) {
                    if (constituentJsonData[j]['Constituent Type']) uniqueConstituents[constituentJsonData[j]['Constituent Type']] = true;
                }
                constituentTypes = Object.keys(uniqueConstituents).sort();

                if (spotlightConfig) {
                    spotlightSourceData = spotlightConfig.type === 'giftAppeal' ? giftAppeals.slice() : constituentTypes.slice();
                }
    ...
    function addCategory() {
        ...
        if (!categoryName) { alert('Please enter an event name'); return; }
        if (categories.length >= 3) { alert('Maximum of 3 events allowed'); return; }
        if (categories.indexOf(categoryName) > -1) { alert('This event already exists'); return; }
    ...
    function startMapping() {
        ...
        scrollToSection('mappingSection');
    }
    ...
    function showCurrentAppeal() {
        ...
        if (currentIndex >= giftAppeals.length) {
            ...
            document.getElementById('giftTypeMappingSection').style.display = 'none';
        ...
        if (currentMapping) {
            ...
            for (var j = 0; j < buttons.length; j++) {
                if (buttons[j].getAttribute('data-category') === currentMapping) {
                    buttons[j].style.background = currentMapping === 'Skip' ? '#999' : '#2c5f5d';
                    buttons[j].style.color = 'white';
                    buttons[j].style.borderColor = currentMapping === 'Skip' ? '#999' : '#2c5f5d';
                }
            }
        }
        var nextBtn = container.querySelector('#nextBtn');
        if (nextBtn && hasUsedPrevious && currentMapping) nextBtn.style.display = 'block';
    }
    ...
    function selectCategory(appeal, category) {
        ...
        for (var i = 0; i < buttons.length; i++) {
            ...
            if (btnCategory === category) {
                buttons[i].style.background = category === 'Skip' ? '#999' : '#2c5f5d';
                buttons[i].style.color = 'white';
                buttons[i].style.borderColor = category === 'Skip' ? '#999' : '#2c5f5d';
            } else {
                buttons[i].style.background = 'white';
                buttons[i].style.color = buttons[i].classList.contains('non-event-btn') ? '#666' : '#2c5f5d';
                buttons[i].style.borderColor = buttons[i].classList.contains('non-event-btn') ? '#999' : '#2c5f5d';
            }
        }
        var nextBtn = document.querySelector('#nextBtn');
        if (nextBtn && hasUsedPrevious) { nextBtn.disabled = false; nextBtn.style.display = 'block'; }
    ...
    function updateProgress() {
        ...
        document.getElementById('progressBarText').textContent = percentage === 0 ? '' : percentage + '%';
    ...
    function startSpotlightMapping() {
        ...
        scrollToSection('spotlightMappingSection');
    }
    ...
    function showCurrentSpotlight() {
        ...
        if (nextBtn && spotlightHasUsedPrevious && currentMapping) nextBtn.style.display = 'block';
    }
    ...
    function selectSpotlight(sourceValue, category) {
        ...
        if (nextBtn && spotlightHasUsedPrevious) { nextBtn.disabled = false; nextBtn.style.display = 'block'; }
    ...
    function updateSpotlightProgress() {
        ...
        var percentage = total > 0 ? Math.round((mapped / total) * 100) : 0;
        document.getElementById('spotlightProgressBar').style.width = percentage + '%';
        document.getElementById('spotlightProgressBarText').textContent = percentage === 0 ? '' : percentage + '%';
    ...
    function startConstituentMapping() {
        ...
        document.getElementById('mappingSection').style.display = 'none';
        document.getElementById('spotlightMappingSection').style.display = 'none';
        scrollToSection('constituentMappingSection');
    }
    ...
    function showCurrentConstituentType() {
        ...
        if (currentMapping) {
            ...
            for (var j = 0; j < buttons.length; j++) {
                if (buttons[j].getAttribute('data-category') === currentMapping) {
                    buttons[j].style.background = '#2c5f5d'; buttons[j].style.color = 'white'; buttons[j].style.borderColor = '#2c5f5d';
                }
            }
        }
        var nextBtn = container.querySelector('#constituentNextBtn');
        if (nextBtn && constituentHasUsedPrevious && currentMapping) nextBtn.style.display = 'block';
    }
    ...
    function selectConstituentType(originalType, mappedType) {
        ...
        for (var i = 0; i < buttons.length; i++) {
            var btnCategory = buttons[i].getAttribute('data-category');
            if (btnCategory === mappedType) { buttons[i].style.background = '#2c5f5d'; buttons[i].style.color = 'white'; buttons[i].style.borderColor = '#2c5f5d'; }
            else { buttons[i].style.background = 'white'; buttons[i].style.color = '#2c5f5d'; buttons[i].style.borderColor = '#2c5f5d'; }
        }
        var nextBtn = document.querySelector('#constituentNextBtn');
        if (nextBtn && constituentHasUsedPrevious) { nextBtn.disabled = false; nextBtn.style.display = 'block'; }
    ...
    function updateConstituentProgress() {
        ...
        var percentage = total > 0 ? Math.round((mapped / total) * 100) : 0;
        document.getElementById('constituentProgressBar').style.width = percentage + '%';
        document.getElementById('constituentProgressBarText').textContent = percentage === 0 ? '' : percentage + '%';
    ...
    function startGiftTypeMapping() {
        ...
        document.getElementById('constituentMappingSection').style.display = 'none';
        scrollToSection('giftTypeMappingSection');
    }
    ...
    function showCurrentGiftType() {
        ...
        if (nextBtn && giftTypeHasUsedPrevious && currentMapping) nextBtn.style.display = 'block';
    }
    ...
    function selectGiftType(originalType, mappedType) {
        ...
        for (var i = 0; i < buttons.length; i++) {
            ...
            if (btnCategory === mappedType) {
                buttons[i].style.background = mappedType === 'Skip' ? '#999' : '#2c5f5d';
                buttons[i].style.color = 'white';
                buttons[i].style.borderColor = mappedType === 'Skip' ? '#999' : '#2c5f5d';
            } else {
                buttons[i].style.background = 'white';
                buttons[i].style.color = buttons[i].classList.contains('non-event-btn') ? '#666' : '#2c5f5d';
                buttons[i].style.borderColor = buttons[i].classList.contains('non-event-btn') ? '#999' : '#2c5f5d';
        ...
        if (nextBtn && giftTypeHasUsedPrevious) { nextBtn.disabled = false; nextBtn.style.display = 'block'; }
    ...
    function updateGiftTypeProgress() {
        ...
        var percentage = total > 0 ? Math.round((mapped / total) * 100) : 0;
        document.getElementById('giftTypeProgressBar').style.width = percentage + '%';
        document.getElementById('giftTypeProgressBarText').textContent = percentage === 0 ? '' : percentage + '%';
    ...
    function generateExcelBlob() {
        ...
        if ((completedMappings + spotlightComplete + constituentComplete + giftTypeComplete) < requiredMappings) return null;
            ...
            if (spotlightConfig.type === 'giftAppeal') {
                for (var j = 0; j < giftJsonData.length; j++) {
                    ...
                    for (var key in mappings) {
                        if (mappings.hasOwnProperty(key) && mappings[key] === eventVal) { appeal = key; break; }
                    }
                ...
                for (var k = 0; k < constituentJsonData.length; k++) {
                    constituentSpotlightMap[constituentJsonData[k]['Constituent ID']] = spotlightMappings[constituentJsonData[k]['Constituent Type']] || 'Unmapped';
                }
                for (var m = 0; m < giftJsonData.length; m++) {
                    giftJsonData[m]['Spotlights'] = constituentSpotlightMap[giftJsonData[m]['Constituent ID']] || 'Unmapped';
                }
            }
        }
        ...
        XLSX.utils.book_append_sheet(newWB, XLSX.utils.json_to_sheet(giftJsonData), 'Gift Data');
        XLSX.utils.book_append_sheet(newWB, XLSX.utils.json_to_sheet(constituentJsonData2), 'Constituent Data');
    ...
    window.attachToGHLForm = function() {
        ...
        if (!excelBlob) {
            alert('Please complete all mapping steps before submitting the form');
            return false;
        }
        var file = new File([excelBlob], 'Gift_Data_with_Events.xlsx', { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
